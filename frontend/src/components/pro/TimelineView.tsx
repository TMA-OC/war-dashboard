          ...
          {sorted.slice(0, 100).map((alert) => {
            ...
            return (
              <div key={alert.id} className="relative pl-8">
                <div className={`absolute left-1.5 top-2 w-3 h-3 rounded-full border-2 border-[#060609] ${
                  alert.isBreaking ? "bg-red-500" :
                  alert.confidenceScore >= 90 ? "bg-green-500" :
                  alert.confidenceScore >= 70 ? "bg-yellow-500" :
                  "bg-gray-600"
                }`} />
                <div className="bg-[#0d0d14] border border-gray-800 rounded-lg p-2.5">
                  <div className="flex items-start justify-between gap-2">
                    <p className="text-xs text-white leading-tight">{alert.headline}</p>
                  ...
                  <div className="flex items-center gap-2 mt-1.5">
                    ...
                    {alert.source && <span className="text-xs text-gray-600">{alert.source.name}</span>}
                  </div>
                </div>
              </div>
            );
